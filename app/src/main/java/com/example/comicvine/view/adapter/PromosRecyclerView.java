...
public class PromosRecyclerView extends RecyclerView.Adapter<PromosRecyclerView.MyHolder> {

    List<IssuesResults> mList;
    Context context;

    public PromosRecyclerView(List<IssuesResults> mList, Context context) {
        this.mList = mList;
    ...
    @NonNull
    @Override
    public PromosRecyclerView.MyHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view= LayoutInflater.from(parent.getContext()).inflate(R.layout.item_promos,parent,false);

        return new MyHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull PromosRecyclerView.MyHolder holder, int position) {

        IssuesResults results=mList.get(holder.getAdapterPosition());

        holder.cTitle.setText(results.getVolume().getName()+ "\n\n");
        holder.iNumber.setText(results.getDeck());
